"""Generate Contextual Situation (CS) sessions from a *defended* profile
parquet file (produced by Defenses/generalize_profiles.py,
create_eeyore_dp_simple.py, or create_eeyore_dp_mix_iter.py). Identical
generation logic to Generation/script_situation.py; the only difference is
that the input parquet is required explicitly via --input_file rather than
the raw eeyore-data.parquet."""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate Contextual Situation (CS) sessions from a defended profile parquet "
    ...
    prompt_text = load_prompt(PROMPTS_DIR, "situation.txt")
    ...
    for i in range(len(profile_list)):
        ...
        if id_list[i] not in source_list and i in valid_indices:
            ...
            client_background_information_i, client_medical_information_i, client_resistance_i, _ = (
                get_client_information(profile_i, mode="situation"))
