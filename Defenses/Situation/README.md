...
Generates Contextual Situation (CS) sessions from a *defended* profile parquet produced by one of the [Defense Algorithms](../README.md#defense-algorithms), instead of the raw `eeyore-data.parquet` used by [`Generation/script_situation.py`](../../Generation/README.md). Otherwise identical generation logic (same `situation.txt` prompt, same `get_client_information(mode="situation")` split, which excludes demographics even though they may be present in the defended profile).
