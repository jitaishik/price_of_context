...
Scripts that build *defended* (anonymized/perturbed) versions of the Eeyore profile data, and a post-hoc defense applied directly to already-generated sessions. All three algorithm scripts below hardcode their input paths to `eeyore-data.parquet`/`valid_indices.json` at the repository root.
...
python Defenses/NoiseProf.py \
    -o eeyore-dp-simple.parquet -model qwen -p 0.2

python Defenses/NoiseIterMix.py \
    -o eeyore-dp-mix-iter.parquet -model qwen -p 0.2
```
...
| `-model` / `--model_name` | Model registered in vLLM (default `llama`) |
...
```bash
# FP sessions
python Defenses/NER.py 'out_fp/*.json' -o out_fp_ner -model qwen

# CS sessions
python Defenses/NER.py 'out_cs/*.json' -o out_cs_ner -model qwen
```
