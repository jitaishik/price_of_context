...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate sessions from pre-built synthetic patient profiles.")
    ...
    parser.add_argument("--profile_source_dir", type=str, required=True,
                        help="Directory of pre-built synthetic profile JSON files "
                             "(e.g. random_synthetic_profiles_qwen or symptoms_synthetic_profiles_qwen, "
                             "produced by random_synthetic_profiles.py / symptoms_synthetic_profiles.py).")
    ...
    for i in range(len(id_list)):
        if id_list[i] not in source_list and i in valid_indices:
            ...
            with open(os.path.join(args.profile_source_dir, f"session_{i+1}.json"), "r") as file_profile:
                profile_dict = json.load(file_profile)
            client_background_information_i = profile_dict["client_background_information"]
            client_medical_information_i = profile_dict["client_medical_information"]
            client_resistance_i = profile_dict["client_resistance"]
            ...
            d_cleaned = {"id": id_list[i], "history": []}
